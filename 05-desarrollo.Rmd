...
1.  Para la pregunta ¿Ha hecho búsquedas en el repositorio Saber UCV? se muestran en la figura \@ref(fig:preguntaa) donde se aprecia que indica ????, lo que conlleva a que

```{r preguntaa, echo=FALSE, fig.align='center', fig.cap='Pregunta: ¿Ha hecho búsquedas en el repositorio Saber UCV?',  out.width="35%"}
...
2.  Los resultados recopilados para la pregunta ¿Pudo realizar una búsqueda en el SCSU? se muestran en la figura \@ref(fig:preguntab) donde se aprecia que indica ????, lo que conlleva a .

```{r preguntab, echo=FALSE, fig.align='center', fig.cap='Pregunta: ¿Pudo realizar una búsqueda en el SCSU?',  out.width="35%"}

pregunta2 <- data.frame(respuesta=c('sí','no'),valor=c(80,20))

labs <-paste0(pregunta2$respuesta," (", pregunta2$valor, " %)")
...
3.  Los resultados capturados para la pregunta ¿Los resultados de un query en el SCSU satisfacen sus expectativas ? se muestran en la figura \@ref(fig:preguntac) donde se aprecia que indica ????, lo que conlleva a .

```{r preguntac, echo=FALSE, fig.align='center', fig.cap='Pregunta: ¿Los resultados de un query en el SCSU satisfacen sus expectativas? ',  out.width="35%"}

pregunta3 <- data.frame(respuesta=c('sí','no'),valor=c(70,30))

labs <-paste0(pregunta3$respuesta," (", pregunta3$valor, " %)")
...
4.  Los resultados recopilados para la pregunta ¿Comprendió las funcionalidades de cada pestaña del SCSU apoyándose en el menú de ayuda y en los tooltips? se muestran en la figura \@ref(fig:preguntad) donde se aprecia que indica ????, lo que conlleva a.

```{r preguntad, echo=FALSE, fig.align='center', fig.cap='Pregunta: ¿Comprendió las funcionalidades de cada pestaña del SCSU?',  out.width="35%"}

pregunta4 <- data.frame(respuesta=c('sí','no'),valor=c(80,20))

labs <-paste0(pregunta4$respuesta," (", pregunta4$valor, " %)")
...
5.  Los resultados obtenidos para la pregunta ¿En caso de ser negativa la respuesta anterior cuál funcionalidad no comprendió? se muestran en la figura \@ref(fig:preguntae) donde se aprecia que indica ????, lo que conlleva a .

```{r preguntae, echo=FALSE, fig.align='center', fig.cap='Pregunta condicionada: ¿Cuál funcionalidad no comprendió?',  out.width="35%"}

pregunta5 <- data.frame(respuesta=c('interfaz',
                                    'mapas de conocimiento',
                                    'inspección mc',
                                    'resultados recomendaciones'),valor=c(20,30,40,20))

labs <-paste0(pregunta5$respuesta," (", pregunta5$valor, " %)")
...
6.  Los resultados que se obtuvieron para la pregunta ¿Los colores de la interfaz del SCSU resultan agradables? ( se muestran en la figura \@ref(fig:preguntaf) donde se aprecia que indica ????, lo que conlleva a .

```{r preguntaf, echo=FALSE, fig.align='center', fig.cap='Pregunta: ¿Los colores de la interfaz del SCSU resultan agradables? ',  out.width="35%"}
pregunta6 <- data.frame(respuesta=c('sí','no'),valor=c(90,10))

labs <-paste0(pregunta6$respuesta," (", pregunta6$valor, " %)")
...
Para realizar esta prueba, en una adaptación de lo revisado en \@ref(relevancia), "Relevancia", a un grupo de 4 expertos en ciencias de la computación se les pidió que evaluaran si cada uno de los resultados obtenidos de cuatro *querys* en el SCSU eran relevantes (de interés). Los datos de la prueba se muestran en la tabla a continuación compuesta por las columas: término del query, la cantidad de resultados obtenidos por query en SCSU, cantidad de documentos relevantes según el juicio de experto (cifra promedio por query) , *accuracy* obtenido por el SCSU (cifra promedio por query).
...
Las cifras obtenidas indican que el desempeño del sistema presenta una exactitud (*accuracy*) promedio de \_\_\_\_.

Es necesario indicar que, procedimentalmente para hacer esta prueba, se realizaron las siguientes consideraciones:

1.  La prueba se hizo exclusivamente con documentos del subcojunto de trabajos de grado catalogados como pertenecientes a la Escuela y el Postgrado de Ciencias de la Computación, ya que sería imposible abordar las distintas áreas de conocimiento que conforman el corpus.

2.  En lo relativo a la selección de los términos de los *querys*, estos se determinaron por la extracción de datos de los resúmenes de los trabajos mediante el método *Rake*, revisado anteriormente en \@ref(fig:compurake), "Prototipo del SCSU".

3.  En lo referente a la válidez de usar *querys* de dos términos, esto se sustenta en que diversos estudios indican que inicialmente las consultas en los sistemas de recuperación de información, en el 80% de los casos, están compuestos por menos de tres palabras [@clarke2000].
